...
public class AudioContext {
     ...
     * net.beadsproject.beads.core.io.NonrealtimeIO otherwise. To get the
     * former, link to the jaudiolibs-beads.jar file.
     * 
     * The libraries are decoupled like this so that the core beads library
     * doesn't depend on JavaSound, which is not supported in various contexts,
     * such as Android. At the moment there are in fact some JavaSound
     * dependencies still to be removed before this process is complete.
     * Pro-users should familiarise themselves with the different IO options,
     * particularly Jack.
     */
    public AudioContext() {
     ...
     * 
     * @param bufferSize
     *            the number of samples calculated during one frame of audio
    ...
    public AudioContext(int bufferSize) {
        ...
        try {
            Class javaSoundAudioIOClass = Class
                    .forName("net.beadsproject.beads.core.io.JavaSoundAudioIO"); // alt
                                                                                 // choice
                                                                                 // is
                                                                                 // org.jaudiolibs.beads.AudioServerIO$JavaSound.
            Constructor noArgsConstructor = javaSoundAudioIOClass
                    .getConstructor();
            ioSystem = (AudioIO) noArgsConstructor.newInstance();
            System.out
                    .println("AudioContext : no AudioIO specified, using default => "
                            + javaSoundAudioIOClass.getName() + ".");
        } catch (Exception e) {
            // if fail, print warning and revert to NonrealtimeIO.
            System.out
                    .println("AudioContext : warning : unable to find default (JavaSound) AudioIO.");
            System.out
                    .println("AudioContext : warning : reverting to NonrealtimeIO. You can still process audio but don't expect to hear anything.");
            ioSystem = new NonrealtimeIO();
        }
        // default audio format
        IOAudioFormat audioFmt = defaultAudioFormat(2, 2);
        // bind to AudioIO
        this.audioIO = ioSystem;
        this.audioIO.context = this;
        // set audio format
        this.audioFormat = audioFmt;
        // set buffer size
        ...
        out = new Gain(this, audioFmt.outputs);
        this.audioIO.prepare();

    }

    /**
     * Creates a new AudioContext with default audio format and buffer size and
     * the specified {@link AudioIO}. The default audio format is 44.1Khz, 16
     * bit, stereo, signed, bigEndian.
     * 
     * @param ioSystem
     *            the AudioIO system.
     */
     ...
     * buffer size and {@link AudioIO}. The default audio format is 44.1Khz, 16
     * bit, stereo, signed, bigEndian.
     * 
    ...
    /**
     * Creates a new AudioContext with the specified buffer size, AudioIO and
     * audio format.
     * 
     * @param bufferSizeInFrames
     *            the buffer size in samples.
     ...
     * each input channel of the audio input device. For access to specific
     * channels see {@link #getAudioInput(int[])}.
     * 
     * @return a UGen which can be used to access audio input.
     */
     ...
     * buffer of zeros that you can actually do something with, use
     * {@link #getCleanBuf()}.
     * 
     * @return buffer of size bufSize, all zeros.
     */
     ...
     * AudioContext's AudioFormat except for the number of inputs and outputs.
     * 
     * @param inputs
     *            num inputs
     * @param outputs
    ...
    public IOAudioFormat getAudioFormat(int inputs, int outputs) {
        IOAudioFormat newFormat = new IOAudioFormat(audioFormat.sampleRate,
                audioFormat.bitDepth, inputs, outputs);
        return newFormat;
    }
    ...
    /**
     * @return the runtime (in ms) since starting.
     */
     ...
     */
    public void logTime(boolean lt) {
        this.logTime = lt;
    }
    ...
    public void record(double timeMS, String filename) throws Exception {
        Sample s = new Sample(timeMS, audioFormat.outputs,
